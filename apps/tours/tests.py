...
class ToursAPITests(TenantAPITestCase):
    def test_vehicle_list_is_scoped_to_organization(self):
        ...
        plates = [row['registration_number'] for row in response.data['results']]
        self.assertEqual(plates, ['KAA-001'])

    def test_cannot_create_booking_for_foreign_customer(self):
        self.client.force_authenticate(self.user_a)
        response = self.client.post('/api/v1/tours/bookings/', {
    ...
    def test_booking_and_assignment_happy_path(self):
        ...
        self.client.force_authenticate(self.user_a)
        booking_response = self.client.post('/api/v1/tours/bookings/', {
        ...
        assignment_response = self.client.post('/api/v1/tours/assignments/', {
            'booking': booking_id,
            'guide': str(guide.id),
            'vehicle': str(vehicle.id),
        }, format='json')
        self.assertEqual(assignment_response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(TourAssignment.objects.filter(booking_id=booking_id).exists())

        self.client.force_authenticate(self.user_b)
        hidden = self.client.get(f'/api/v1/tours/bookings/{booking_id}/')
    ...
    def test_assignment_rejects_foreign_vehicle(self):
        ...
        foreign_vehicle = Vehicle.objects.create(
            organization=self.org_b,
            registration_number='KBB-999',
            make='Toyota',
            model='Hiace',
        )

        self.client.force_authenticate(self.user_a)
        response = self.client.post('/api/v1/tours/assignments/', {
